...
window.navermap_authFailure = function () {
  console.error('NAVER_MAP_AUTH_FAILURE');
};
...
function formatMaintenanceFee(value) {
  ...
  return `관리비 ${formatAmount(display)}`;
}
...
function waitForNaverMapsSdkReady(resolve, reject) {
  const startedAt = Date.now();

  const checkReady = () => {
    if (isNaverMapsSdkReady()) {
      resolve();
      return;
    }

    if (Date.now() - startedAt > 10000) {
      reject(new Error('Naver Maps SDK did not finish loading.'));
      return;
    }
...
function loadNaverMapsSdk() {
  if (isNaverMapsSdkReady()) return Promise.resolve();
  if (naverMapsSdkPromise) return naverMapsSdkPromise;

  naverMapsSdkPromise = new Promise((resolve, reject) => {
    const finishLoading = () => waitForNaverMapsSdkReady(resolve, reject);
...
function normalizeKoreaCoordinatePair(latValue, lngValue) {
  if (isBlankCoordinateValue(latValue) || isBlankCoordinateValue(lngValue)) return null;
  const lat = Number(latValue);
  const lng = Number(lngValue);
  if (!Number.isFinite(lat) || !Number.isFinite(lng)) return null;
  if (lat === 0 || lng === 0) return null;
...
async function geocodeAddressWithNaver(address) {
  ...
  const primaryQuery = query.replace(/\s*\([^)]*\)\s*$/, '').trim();
  const queries = [primaryQuery || query];

  if (query.includes('인동35길 29-1') || query.includes('인의동 991-3')) {
    queries.push('경상북도 구미시 인의동 991-3');
  }

  const geocodeWithApi = async (targetQuery) => {
    const response = await fetch(`/api/geocode-address?address=${encodeURIComponent(targetQuery)}`);
    const data = await response.json().catch(() => ({}));
    if (response.ok) {
      const coords = normalizeCoordinatePair(data.latitude, data.longitude);
      if (coords) return { ...coords, source: 'naver_geocode' };
    }
    return null;
  };

  const geocodeWithSdk = (targetQuery) => loadNaverGeocoderScript().then(() => new Promise((resolve) => {
    if (!window.naver?.maps?.Service?.geocode) {
      resolve(null);
      return;
    }

    window.naver.maps.Service.geocode({ query: targetQuery }, (status, response) => {
  ...
  for (const targetQuery of queries) {
    try {
      const apiCoords = await geocodeWithApi(targetQuery);
      if (apiCoords) return apiCoords;
    } catch (error) {
      // Fall through to the browser map SDK when the local API is unavailable.
    }

    const sdkCoords = await geocodeWithSdk(targetQuery);
    if (sdkCoords) return sdkCoords;
  }
...
function getPropertyMapPoint(property = {}, index = 0) {
  const coords = normalizeKoreaCoordinatePair(property.latitude, property.longitude);
  ...
  if (
    isBlankCoordinateValue(property.latitude) ||
    isBlankCoordinateValue(property.longitude)
  ) {
    return null;
  }
...
function CustomerMapView({ properties, selected, onSelect, keyword, setKeyword, dealMode, setDealMode, category, setCategory, filters, setFilters }) {
  ...
  })).filter((item) => item.point), [properties]);
              ...
              <button
                ...
                onClick={() => {
                  ...
                  if (coords) {
                    ...
                    updateField('coords_source', coords.source || 'address_search');
                  }
                  if (isNewPropertyRegistration) {
                    if (!coords) {
                      updateField('latitude', '');
...
const payloadCoords = normalizeKoreaCoordinatePair(payload.latitude, payload.longitude);
...
if (needsGeocode) {
  ...
  const validCoords = coords ? normalizeKoreaCoordinatePair(coords.lat, coords.lng) : null;

  if (validCoords) {
    payload.latitude = validCoords.lat;
    payload.longitude = validCoords.lng;
    payload.geocode_status = 'success';
    payload.geocoded_at = new Date().toISOString();
    payload.coords_source = coords.source || 'naver_geocode';
  } else {
    setStatus('저장 중단: 주소를 지도 좌표로 변환하지 못했습니다. 주소를 다시 확인해주세요.');
    return;
  }
...
const request = editingId && canEditExisting
  ...
  : editingId && isStaffMode && currentStaff?.code
    ? supabase
        ...
        .select('id,address,latitude,longitude,status')
        .single()
    : supabase.from('properties').insert(insertPayload).select('id,address,latitude,longitude,status').single();
   ...
   const { data: savedProperty, error } = await request;
    ...
    const savedCoords = normalizeKoreaCoordinatePair(savedProperty?.latitude, savedProperty?.longitude);
    if (!savedProperty?.id || !savedProperty?.address || !savedCoords) {
      setStatus('저장 후 좌표 확인 실패: latitude/longitude가 정상 숫자로 저장되지 않았습니다.');
      return;
    }
    if (isStaffMode) {
    ...
    setStatus(geocodeWarning || (isStaffMode ? '임시저장 완료되었습니다. 대표 검수 후 홈페이지에 노출됩니다.' : (editingId ? '수정 완료되었습니다.' : '등록 완료되었습니다.')));
   resetForm();
await reload();
...
function buildNaverBlogAdLegacy(property) {
  ...
  const uniqueList = (values) => [...new Set(values.map(compact).filter(Boolean))];
  const options = uniqueList(toList(property.convenience)).filter(
    (item) => !/(인근|생활권|출퇴근|통근|버스|편의점|마트|병원|학교|산업단지)/u.test(item)
  );
  const maintenanceItems = uniqueList(
    toList(property.maintenance_includes).map((item) => item.replace(/수고요금/g, '수도요금'))
  );
  const maintenanceFeeText = clean(property.maintenance_fee).replace(/수고요금/g, '수도요금');
  ...
  const propertyType = compact(
    category
      .replace(/월세|전세|반전세|매매/g, '')
      .replace(/\s+/g, ' ')
  ) || (isSale ? '건물' : '매물');

  const rawParking = clean(property.parking);
  const parkingCountMatch = rawParking.match(/(?:총\s*)?\d+(?:\.\d+)?\s*대/u);
  const parkingTotal = parkingCountMatch
    ? `총 ${parkingCountMatch[0].replace(/총\s*/u, '').trim()}`
    : '';
  const sanitizeNarrative = (value) => {
    const text = clean(value);
    if (!text || parkingTotal) return text;

    return text
      .split(/(?<=[.!?])\s+/u)
      .filter((sentence) => !/주차(?:가|는)?\s*(?:가능|편리)|주차\s*편리/u.test(sentence))
      .join(' ')
      .trim();
  };
  ...
  const getDong = (value) => {
    ...
    const matches = [...text.matchAll(/([가-힣0-9]+(?:동|읍|면|리))/g)];
    return matches.length ? matches[matches.length - 1][1] : '';
  };

  const dong = getDong(address);
  const locationTitle = dong ? `구미 ${dong}` : '구미';
  const locationSentence = dong ? `구미시 ${dong}` : '구미시';

  const moneyText = (value) => {
    const text = clean(value);
    if (!text) return '';
    if (text.includes('억') || text.includes('만') || text.includes('원')) return text;
    if (/^\d+(\.\d+)?$/.test(text.replaceAll(',', ''))) {
      const amount = Number(text.replaceAll(',', ''));
      if (amount >= 10000) {
        const eok = Math.floor(amount / 10000);
        const manwon = amount % 10000;
        return manwon ? `${eok}억 ${manwon.toLocaleString()}만원` : `${eok}억원`;
      }
  ...
  const priceText = isSale
    ? (salePrice ? `매매가 ${moneyText(salePrice)}` : '매매가 계약 전 확인')
    : [
  ...
  const usablePrice = (value) => {
    const text = clean(value);
    return text && !/확인|문의|상담|입력/u.test(text) ? text : '';
  };
  const titleSalePrice = usablePrice(salePrice);
  const titleTakeoverPrice = usablePrice(takeoverPrice);
  const titleMonthlyRent = usablePrice(monthlyRent);
  const titleNetIncome = usablePrice(netIncome);
  const rentalPriceTitle = [
    titleMoney(property.deposit),
    titleMoney(property.rent)
  ].filter(Boolean).join('/');

  const titleBenefitText = (value) => {
    const phrase = compact(value)
      .split(/[.,。\n]/u)[0]
      .replace(/(?:으로|로) 출퇴근.*$/u, '')
      .replace(/ 임대수요.*$/u, ' 임대수요')
      .replace(/(?:입니다|합니다|있습니다)$/u, '')
      .trim();
    const nearbyPhrase = phrase.match(/^(.{2,40}?인근)/u);
    if (nearbyPhrase) return nearbyPhrase[1];
    return phrase.length > 32 ? phrase.slice(0, 32).trim() : phrase;
  };
  const rentalBenefit = [
  ...
  const saleBenefit = [
    titleBenefitText(property.investment_point),
    titleBenefitText(property.location_description),
    titleBenefitText(property.building_condition),
    titleBenefitText(property.remodeling),
  ...
  const automaticTitle = compact(
    isSale
      ? [
          ...
          !titleMonthlyRent && titleNetIncome ? `월순수익 ${moneyText(titleNetIncome)}` : '',
          saleBenefit
        ].filter(Boolean).join(' ')
      : [
          `${locationTitle} ${propertyType} ${tradeType}`,
  ...
  const section = (heading, lines) => {
    const content = [...lines];
    while (content.length && !clean(content[0])) content.shift();
    while (content.length && !clean(content[content.length - 1])) content.pop();
    if (!content.some((line) => clean(line))) return [];
  ...
  const maintenanceText = maintenanceItems.length ? maintenanceItems.join(', ') : '';
  const introduction = isSale
    ? [
        `오늘 소개드릴 매물은 ${locationSentence}에 위치한 ${propertyType}입니다.`,
        saleBenefit
          ? `${priceText} 조건이며, ${saleBenefit}이라는 장점이 있습니다.`
          : `${priceText} 조건으로 안내드리는 건물입니다.`
      ]
    : [
        `오늘 소개드릴 매물은 ${locationSentence}에 위치한 ${propertyType} ${tradeType} 매물입니다.`,
        [priceText, sanitizeNarrative(property.summary) || sanitizeNarrative(property.title)].filter(Boolean).join(' 조건이며, ')
      ];
  ...
  const legalNoticeText = isSale
    ? sanitizeNarrative(property.legal_notice)
    : sanitizeNarrative(property.legal_notice)
        .split(/(?<=[.!?])\s+/u)
        .filter((sentence) => !/(해당\s*층|관리비\s*월|전용면적)/u.test(sentence))
        .join(' ')
        .trim();
  ...
  const rentalComposition = [
    clean(property.room_count) ? `원룸 ${clean(property.room_count)}` : '',
    clean(property.mini_two_count) ? `미니투룸 ${clean(property.mini_two_count)}` : '',
    clean(property.two_room_count) ? `투룸 ${clean(property.two_room_count)}` : '',
  ...
  const locationLines = [
    sanitizeNarrative(property.location_description)
  ];

  const structureLines = isSale
    ? [
        clean(property.description),
        clean(property.land_area) ? `대지면적: ${clean(property.land_area)}` : '',
        clean(property.total_area) ? `건축면적: ${clean(property.total_area)}` : '',
        clean(property.building_area) ? `연면적: ${clean(property.building_area)}` : '',
        currentFloor ? `해당 층: ${currentFloor}` : '',
        totalFloor ? `총층수: ${totalFloor}` : '',
        clean(property.approval_date) ? `사용승인일: ${clean(property.approval_date)}` : '',
        ...
        ownerUnitText ? `주인세대: ${ownerUnitText}` : '',
        parkingTotal ? `총주차대수: ${parkingTotal}` : '',
        clean(property.remodeling) ? `리모델링: ${clean(property.remodeling)}` : '',
        clean(property.building_condition) ? `건물 상태: ${clean(property.building_condition)}` : '',
        clean(property.total_units) ? `총 세대수: ${clean(property.total_units)}` : '',
        clean(property.rented_units) ? `임대중 세대수: ${clean(property.rented_units)}` : '',
        clean(property.vacant_units) ? `공실 수: ${clean(property.vacant_units)}` : '',
  ...
  const conditionLines = isSale
    ...
    : [
        maintenanceFeeText ? `관리비: ${maintenanceFeeText}` : '',
        maintenanceText ? `관리비 포함 항목: ${maintenanceText}` : '',
        parkingTotal ? `총주차대수: ${parkingTotal}` : '',
        clean(property.move_in) ? `입주가능일: ${clean(property.move_in)}` : ''
      ];

  const recommendationLines = isSale
    ? [
        clean(property.investment_point) || clean(property.recommended_for)
      ]
    : [recommendText];

  const legalInfoLines = isSale
    ? [
        addInfo('중개대상물 종류', category),
        addInfo('거래형태', tradeType),
        addInfo('소재지', address),
        addInfo('거래가격', salePrice ? `매매가 ${moneyText(salePrice)}` : ''),
        takeoverPrice ? addInfo('인수금', moneyText(takeoverPrice)) : '',
        monthlyRent ? addInfo('월세수입', moneyText(monthlyRent)) : '',
        netIncome ? addInfo('월순수익', moneyText(netIncome)) : '',
        clean(property.land_area) ? addInfo('대지면적', property.land_area) : '',
        clean(property.total_area) ? addInfo('건축면적', property.total_area) : '',
        clean(property.building_area) ? addInfo('연면적', property.building_area) : '',
        currentFloor ? addInfo('해당 층', currentFloor) : '',
        totalFloor ? addInfo('총층수', totalFloor) : '',
        clean(property.approval_date) ? addInfo('사용승인일', property.approval_date) : '',
        clean(property.structure) ? addInfo('건물구조', property.structure) : '',
        clean(property.total_units) ? addInfo('총세대수', property.total_units) : '',
        parkingTotal ? addInfo('총주차대수', parkingTotal) : '',
        clean(property.elevator) ? addInfo('엘리베이터', property.elevator) : '',
        directionText ? addInfo('방향', directionText) : '',
        ownerUnitText ? addInfo('주인세대', ownerUnitText) : '',
        rentalComposition ? addInfo('임대구성', rentalComposition) : ''
      ]
    : [
        addInfo('중개대상물 종류', category),
        addInfo('거래형태', tradeType),
        ...
        addInfo('관리비', maintenanceFeeText),
        addInfo('면적', property.area),
        addInfo('해당 층', currentFloor),
        addInfo('총층수', totalFloor),
  ...
  const bodyLines = [
    ...
    '',
    ...introduction.filter(Boolean),
    '',
    ...section(`🏠 ${locationTitle} ${propertyType} ${tradeType} 기본정보`, basicInfo.filter(Boolean)),
    '',
    ...section('📍 위치와 생활권', locationLines),
    '',
    ...section(isSale ? '🏢 건물 구성과 상태' : '🛋️ 구조와 옵션', structureLines.filter(Boolean)),
    '',
    ...
    ...section('🌐 칸공인중개사 홈페이지·앱 안내', [
      '더 많은 구미 원룸·미니투룸·투룸 임대와 상가·다가구·원룸건물 매매 매물은 칸공인중개사 홈페이지에서 확인하실 수 있습니다.',
      '',
      '홈페이지: https://kan-homepage-final.vercel.app',
      '',
      '스마트폰에서는 “칸 공인중개사” 앱을 이용하시면 매물 확인과 전화·문자·상담 문의를 더욱 편리하게 이용하실 수 있습니다.'
    ]),
    '',
    ...section('📞 전화·문자·상담 문의', [
  ...
  const tagSeeds = isSale
    ? [
        '구미부동산',
        '칸공인중개사',
        dong ? `구미${dong}` : '',
        dong && propertyType ? `${dong}${propertyType}매매` : '',
        propertyType.includes('원룸') ? '구미원룸건물매매' : '',
        propertyType.includes('다가구') ? '구미다가구매매' : '',
        propertyType.includes('상가') ? '구미상가매매' : '',
        '구미건물매매',
        '구미수익형부동산'
      ]
    : [
        ...
        '칸공인중개사',
        dong ? `구미${dong}` : '',
        propertyType ? `구미${propertyType}` : '',
        dong && propertyType ? `${dong}${propertyType}` : '',
        tradeType.includes('월세') ? '구미월세' : '',
        `${makeTag(propertyType)}${makeTag(tradeType)}`
      ];
